...
<%
  ...
  //获取字段名
  ArrayList al1 = GzcxBO.getCurDbfld("prizedetailtpl",userSession);
  //获取字段显示名称
  ArrayList al2 = GzcxBO.getCurFldname("prizedetailtpl",userSession);
  //获取字段显示宽度
  ArrayList al3 = GzcxBO.getCurFldwidth("prizedetailtpl",userSession);
  ArrayList al5 = GzcxBO.getNoShowDbfld("prizedetailtpl", userSession);
...
function prizeDetail()
{
	...
	Efs.Common.ajax("<%=rootPath%>/gzcx_getCreateGridJson.action?tpl=prizedetailtpl"+"&cxnd="+sYear+sMonth+"&zeroflag="+sZeroFlag,"",function(succ,response,options){
    if(succ)    // 是否成功
    {
     	var returnJson = eval("(" + response.responseText + ")");
     	//判断是否有记录
     	if(returnJson.nodeAry.length<1){
     		alert("该年月月奖明细无记录,请重新输入!");
...
<body>
<div region="north" height="60" title="月奖发放明细" collapsible="false">
...
<tr>
	...
	<td>
		<input type="button" value="查 询" onClick="prizeDetail();" iconCls="icon-qry">
	</td>
</tr>
</table>
	<input type="hidden" name="tpl" fieldname="TPL" value="prizedetailtpl"/>
