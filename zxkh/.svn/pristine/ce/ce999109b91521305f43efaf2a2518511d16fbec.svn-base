...
<%
  ...
  ArrayList al4 = GzcxBO.getCxnd("otherdetailtpl",userSession);
  ArrayList al1 = GzcxBO.getCurDbfld("otherdetailtpl",userSession);
  ArrayList al2 = GzcxBO.getCurFldname("otherdetailtpl",userSession);
  ArrayList al3 = GzcxBO.getCurFldwidth("otherdetailtpl",userSession);
  ArrayList al5 = GzcxBO.getNoShowDbfld("otherdetailtpl", userSession);
...
function wageDetail()
{
	...
	Efs.Common.ajax("<%=rootPath%>/gzcx_getCreateGridJson.action?tpl=otherdetailtpl"+"&cxnd="+sYear+sMonth+"&zeroflag="+sZeroFlag,"",function(succ,response,options){
    if(succ)    // 是否成功
    {
     	var returnJson = eval("(" + response.responseText + ")");
     	//判断是否有记录
     	if(returnJson.nodeAry.length<1){
     		alert("该年月其他明细无记录,请重新输入!");
...
<body>
<div region="north" height="60" title="其他发放明细" collapsible="false">
<form id="frmQry"  method="post" action="">
...
<tr>
	...
	<input type="hidden" id="tpl" name="tpl" fieldname="TPL" value="otherdetailtpl"/>
