...
Efs.onReady(
  function(){ 
    Efs.getDom("mgStore").txtXML = Efs.Common.getQryXml();
...
function doSubmit()
{
  ...
  var opObj = Efs.Common.getOpXml(Efs.getExt("frmPost"),true);
  ...
  for(var i=0;i<objXML.selectNodes("//ORDPRO").length ; i++)
  {
    objXML.selectNodes("//ORDPRO/@operation").item(i).text = "0";
    objXML.selectNodes("//ORDPRO/PID/@state").item(i).text = "0";
    
    var newElem = objXML.createElement("ORDERID");
    newElem.setAttribute("state","0");
    newElem.setAttribute("datatype","0");
    objXML.selectNodes("//ORDPRO").item(i).appendChild(newElem); 
  }
    
  objXML.documentElement.appendChild(opObj.selectSingleNode("//ORDERLIST"));
  
  Efs.getExt("frmPost").submit(objXML.xml);
}
