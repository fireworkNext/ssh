...
<%
  ...
  ArrayList al1 = GzcxBO.getCurDbfld("yeardetailtpl",userSession);
  ArrayList al2 = GzcxBO.getCurFldname("yeardetailtpl",userSession);
  ArrayList al3 = GzcxBO.getCurFldwidth("yeardetailtpl",userSession);
  ArrayList al5 = GzcxBO.getNoShowDbfld("yeardetailtpl", userSession);
...
function wageDetail()
{
	...
	Efs.Common.ajax("<%=rootPath%>/gzcx_getCreateGridJson.action?tpl=yeardetailtpl"+"&cxnd="+sYear+sMonth+"&zeroflag="+sZeroFlag,"",function(succ,response,options){
    if(succ)    // 是否成功
    {
     	var returnJson = eval("(" + response.responseText + ")");
     	//判断是否有记录
     	if(returnJson.nodeAry.length<1){
     		alert("该年月年终奖明细无记录,请重新输入!");
...
<body>
<div region="north" height="60" title="年终奖发放明细" collapsible="false">
...
<tr>
	...
	<input type="hidden" id="tpl" name="tpl" fieldname="TPL" value="yeardetailtpl"/>
