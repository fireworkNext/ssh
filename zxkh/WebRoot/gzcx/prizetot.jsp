...
<%
  ...
  int i=0;
  ArrayList al4 = GzcxBO.getCxnd("prizedetailtpl",userSession);
  ArrayList al6 = GzcxBO.getSum("prizedetailtpl",userSession);
  //获取字段名
  ArrayList al1 = GzcxBO.getCurDbfld("prizetottpl",userSession);
  //获取字段显示名称
  ArrayList al2 = GzcxBO.getCurFldname("prizetottpl",userSession);
  //获取字段显示宽度
  ArrayList al3 = GzcxBO.getCurFldwidth("prizetottpl",userSession);
  ArrayList al5 = GzcxBO.getNoShowDbfld("prizetottpl", userSession);
...
Efs.onReady(
  function(){	
	...
	var headerAry;
	nodeAry=<%=sNodeAry%>;
	headerAry=<%=sHeaderAry%>;
	colmWidAry=<%=sColmWidAry%>;
...
<body>
<div region="north" height="60" title="月奖汇总信息" collapsible="false">
...
<tr>
	...
	<td>
		<select id="cxnd" name="cxnd" onChange="" fieldname="YEAR">
        	<%
	...
	<td>
		<input type="button" value="查 询" onClick="prizeTot();" iconCls="icon-qry">
	</td>
	<td>
	...
	<input type="hidden" name="tpl" fieldname="TPL" value="prizetottpl"/>
	<input type="hidden" name="tpl" fieldname="TPLDETAIL" value="prizedetailtpl"/>
</form>
</div>
